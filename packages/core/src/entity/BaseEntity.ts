...
import { AnyEntity, Dictionary, EntityData, IWrappedEntity, LoadedReference, Populate, HelperType } from '../typings';
...
export abstract class BaseEntity<T extends AnyEntity<T>, PK extends keyof T> implements IWrappedEntity<T, PK> {
  ...
  isInitialized(): boolean {
    return (this as unknown as T)[HelperType]!.__initialized;
  }

  populated(populated = true): void {
    (this as unknown as T)[HelperType]!.populated(populated);
  }
  ...
  toObject(ignoreFields: string[] = []): Dictionary {
    return (this as unknown as T)[HelperType]!.toObject(ignoreFields) as EntityData<T>;
  }
  ...
  init(populated = true): Promise<T> {
    return (this as unknown as T)[HelperType]!.init(populated);
  }
